...
exports.startRecording = function(minutesToRecord, callback) {
	...
	var u = {unix: {c: 'rec', a: ['--buffer', 16384, tempFileName]}, win: {c: process.env.comspec, a: ['/c', 'sox', '-t', 'waveaudio', '-d', '--buffer', 16384, tempFileName]}};
	var n = u[f[process.platform]];
	var recording = spawn(n.c, n.a, {cwd: recordings, stdio: 'inherit'});
	...
	setTimeout(function endRecording() {
		...
		if (process.platform === 'win32') {
			spawn('taskkill', ['/pid', recording.pid, '/f', '/t'], {cwd: recordings});
		} else {
			recording.kill();
		}
...
exports.convertMP3 = function(callback) {
	...
	var convert = spawn('lame', [wavFile, mp3File], {cwd: recordings, stdio: 'inherit'});
